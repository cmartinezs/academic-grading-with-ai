...
Each operation opens its own connection with proper pragmas,
performs the transaction, and closes the connection.
...
class EmailLedger:
        ...
        try:
            with conn:
                existing = conn.execute(
                    "SELECT planId FROM plans WHERE planId = ?",
                    (plan_id,)
                ).fetchone()
                if existing is not None:
                    row = conn.execute(
                        """SELECT sectionId, publicationId, snapshotContentHash, snapshotReviewHash,
                    ...
                    if row is not None:
                        ...
                        new_data = (
                            section_id, publication_id, snapshot_content_hash, snapshot_review_hash,
                    ...
                    raise PlanExistsError(
                        f"A different plan already exists with planId {plan_id}"
                    )
                conn.execute(
                    """INSERT INTO plans
            ...
            raise LedgerError(f"Ledger interface error: {exc}") from exc
        except sqlite3.OperationalError as exc:
            raise LedgerError(f"Ledger operational error: {exc}") from exc
        finally:
            conn.close()
    ...
    def get_plan(self, plan_id: str) -> Optional[dict]:
        ...
        try:
            row = conn.execute(
                """SELECT planId, sectionId, publicationId, snapshotContentHash, snapshotReviewHash,
                          snapshotMode, templateId, templateVersion, templateHash, intent,
                          senderProfileId, fromAddress, replyTo, previewHash, recipientCount,
                          planPath, createdAt, status
                   FROM plans WHERE planId = ?""",
            ...
            if row is None:
                return None
            return {
                "planId": row[0], "sectionId": row[1], "publicationId": row[2],
                "snapshotContentHash": row[3], "snapshotReviewHash": row[4],
                "snapshotMode": row[5], "templateId": row[6], "templateVersion": row[7],
                "templateHash": row[8], "intent": row[9], "senderProfileId": row[10],
                "fromAddress": row[11], "replyTo": row[12], "previewHash": row[13],
                "recipientCount": row[14], "planPath": row[15], "createdAt": row[16],
                "status": row[17],
            }
        finally:
            conn.close()
            ...
            with conn:
                cursor = conn.execute(
                    """INSERT INTO deliveries
                       (planId, studentId, idempotencyKey, maskedRecipient,
                        identityProjectionHash, state, attemptCount, clientMessageId, createdAt, updatedAt)
                       VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (plan_id, student_id, idempotency_key, masked_recipient,
    ...
    def acquire_for_execution(self, plan_id: str, student_id: str,
                              idempotency_key: str,
                              masked_recipient: str, identity_projection_hash: str,
                              client_message_id: str) -> tuple[str, Optional[int]]:
        """Atomic reserve-or-skip operation.

        Returns (action, deliveryId) where action is one of:
        - "reserved": new reservation, deliveryId is set
        - "sent": already sent, skip
        - "ambiguous": ambiguous state, block
        - "failedPermanent": permanent failure, block
        - "sending": in-progress, block
        - "failedTransient": needs retryAuthorization first
        - "paused": paused, block
        - "retryAuthorized": ready for retry, deliveryId is set
        """
        now = self._now()
        conn = self._open_conn()
        try:
            with conn:
                existing = conn.execute(
                    """SELECT deliveryId, state FROM deliveries
                       WHERE idempotencyKey = ?""",
                    (idempotency_key,)
                ).fetchone()
                if existing is not None:
                    delivery_id = existing[0]
                    state = existing[1]
                    if state == "sent":
                        return ("sent", delivery_id)
                    if state == "ambiguous":
                        return ("ambiguous", delivery_id)
                    if state == "failedPermanent":
                        return ("failedPermanent", delivery_id)
                    if state == "sending":
                        return ("sending", delivery_id)
                    if state == "paused":
                        return ("paused", delivery_id)
                    if state == "failedTransient":
                        return ("failedTransient", delivery_id)
                    if state == "retryAuthorized":
                        conn.execute(
                            "UPDATE deliveries SET state = ?, updatedAt = ? WHERE deliveryId = ? AND state = ?",
                            ("reserved", now, delivery_id, "retryAuthorized")
                        )
                        return ("retryAuthorized", delivery_id)
                    if state == "reserved":
                        return ("reserved", delivery_id)
                    return ("blocked", delivery_id)
                ...
                return ("reserved", cursor.lastrowid)
        except sqlite3.IntegrityError as exc:
            raise IdempotencyConflictError(
                f"Duplicate idempotencyKey: {idempotency_key}"
            ) from exc
    ...
    def get_delivery_by_key(self, idempotency_key: str) -> Optional[DeliveryRecord]:
        ...
        try:
            row = conn.execute(
                """SELECT deliveryId, planId, studentId, idempotencyKey,
            ...
            return DeliveryRecord(
                ...
                masked_recipient=row[4], identity_projection_hash=row[5],
                state=DeliveryState(row[6]), attempt_count=row[7],
                last_attempt_at=row[8], provider_message_id=row[9],
                client_message_id=row[10], created_at=row[11], updated_at=row[12]
            )
        finally:
            conn.close()
        ...
        try:
            ...
            return [
                DeliveryRecord(
                    ...
                    idempotency_key=r[3],
                    masked_recipient=r[4], identity_projection_hash=r[5],
                    state=DeliveryState(r[6]), attempt_count=r[7],
                    last_attempt_at=r[8], provider_message_id=r[9],
                    client_message_id=r[10], created_at=r[11], updated_at=r[12]
                )
                for r in rows
            ]
        finally:
    ...
    def integrity_check(self) -> list[str]:
        ...
        try:
            ...
            try:
                valid_states = tuple(s.value for s in DeliveryState)
                rows = conn.execute(
                    f"SELECT deliveryId, state FROM deliveries WHERE state NOT IN ({','.join('?' * len(valid_states))})",
                    valid_states
                ).fetchall()
                for row in rows:
